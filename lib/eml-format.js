...
function encodingOfCharset(charset) { //to skip conversion in iconv
  //encoding for iconv, eq. 'iso-8859-2' -> 'iso88592'
...
var emlformat = {
  ...
  charsetDefault: 'iso-8859-2', //or 'utf-8' ... - to use if charset=... is missing
  ...
  getCharset: function(contentType) { //eq. contentType='.....charset="iso-8859-2"....'  //AK
  ...
  //old: var regex = /=\?UTF\-8\?(B|Q)\?(.+?)(\?=)/gi;
    var regex      = /=\?([^?]+)\?(B|Q)\?(.+?)(\?=)/gi;
    var match = regex.exec(s);
    if (match) {
      var encodingChr = encodingOfCharset(match[1] || emlformat.charsetDefault); //eq. match[1] = 'iso-8859-2'; encodingChr = 'iso88592'
  ...
  unquotePrintable: function(s, encodingChr) {
    ...
    if (encodingChr == "utf8") {
      ...
      return s
      .replace(/=([\w\d]{2})/gi, function(matcher, p1, offset, string) { return iconv.decode(Buffer.from([ parseInt(p1, 16) ]), encodingChr); })
      .replace(/=\r?\n/gi, ""); //Join line
    }
  }
};
  ...
  function _read(data) {
    try {
      ...
      if (data.headers["From"]) {
        ...
        result.from = data.headers["From"];
        result.from = result.from.replace(/=\?.+?\?=/g, function(s) { return emlformat.unquoteConv(s); });
      }
      if (data.headers["To"]) {
        //result.to = emlformat.getEmailAddress(data.headers["To"]);
        result.to = data.headers["To"];
        result.to = result.to.replace(/=\?.+?\?=/g, function(s) { return emlformat.unquoteConv(s); });
      }
      ...
      function _append(headers, content) {
        ...
        if (!result.html && contentType && contentType.indexOf("text/html") >= 0) {
          if (typeof content != "string") {
            content = iconv.decode(Buffer.from(content), encodingChrBound);//old: content = content.toString("utf8");
          }
          //Message in HTML format
          result.html = content;
        }
          ...
          if (name) {
            //AK "=?UTF-8?Q?a.zip?="
            name = name.replace(/=\?.+?\?=/g, function(s) { return emlformat.unquoteConv(s); });
  ...
  function _unpack(data) {
    try {
      ...
      if (typeof data.html == "string") {
        ...
        data_html = data_html.replace(/(<body[^>]*>)/i,
          '$1<div class="FromTo" style="background-color: LightBlue;"><p>'
          +'<b>From: </b>'+emailToHtml(data.from)+'<br/>'
          +'<b>_To_: </b>'+emailToHtml(data.to)+'<br/>'
          +'<b>Date: </b>'+data.date.toLocaleString(undefined, dateOptions)+'<br/>'
          +'<b>Subj: </b>'+data.subject
          +'</p></div>');
      ...
      if ((!(emlformat.noTextIfHtml) || !(typeof data.html == "string")) 
        ...
        var txtName = attPadStr+"index.txt";
        result.files.push(txtName);
        //discovery of EOL
        var EOL = '\n';
        var match = /([\r\n]{1,2})/.exec(data.text);
        if (match) 
