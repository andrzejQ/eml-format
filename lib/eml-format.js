...
var emlformat = {
  ...
  unquotePrintable: function(s, encodingChr) {
    ...
    if (encodingChr == "utf8") { //2-bytes starts with /[CD]/...., 3-bytes starts with /E/, 4-bytes: /F[0-7]/
      return s
      .replace(/=(F[0-7])=([A-F0-9]{2})=([A-F0-9]{2})=([A-F0-9]{2})/gi, function(matcher, p1, p2, p3, p4, offset, string) { return new Buffer([ parseInt(p1, 16), parseInt(p2, 16), parseInt(p3, 16), parseInt(p4, 16) ]).toString("utf8"); })
      .replace(/=(E[A-F0-9])=([A-F0-9]{2})=([A-F0-9]{2})/gi, function(matcher, p1, p2, p3, offset, string) { return new Buffer([ parseInt(p1, 16), parseInt(p2, 16), parseInt(p3, 16) ]).toString("utf8"); })
      .replace(/=([CD][A-F0-9])=([A-F0-9]{2})/gi, function(matcher, p1, p2, offset, string) { return new Buffer([ parseInt(p1, 16), parseInt(p2, 16) ]).toString("utf8"); })
